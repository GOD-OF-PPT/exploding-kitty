...
export function body(content: string, scroll = false, className = "sceneBody"): string {
  ...
  return scroll
    ? `<scrollview class="${classes}" scrollY="true">${content}</scrollview>`
...
export function actions(
  ...
  return `<view class="actionDock actionDock${capitalize(layout)}">${values.map((action, index) => {
    ...
    const iconTone = tone === "ink" || tone === "red" ? "cream" : "ink";
    return `<button id="action-${index}" class="actionButton actionTone${capitalize(tone)} ${hierarchy}">${icon(actionIcon, iconTone, "actionIcon")}<text class="actionLabel" value="${escapeMarkup(action.label)}"></text></button>`;
  }).join("")}</view>`;
}
...
export function fitForSource(source: string | undefined): FitMode {
  if (!source) return "contain";
  return source.includes("/cards/") ? "cover" : "contain";
}

function rowTemplate(row: ScreenRow, index: number, variant: RowVariant): string {
  const image = row.image
    ? fitImage(row.image, variant === "seat" ? "seatAvatar" : variant === "rank" ? "rankAvatar" : "rowImage", fitForSource(row.image), "center center")
    : variant === "menu" || variant === "setting" || variant === "fact"
      ? icon(rowIcon(row), "ink", "rowIcon")
      : variant === "timeline"
        ? icon("arrow-clockwise", "cream", "timelineIcon")
        : "";
  const caret = row.action ? icon("caret-right", "ink", "rowCaret") : "";
  return `<button id="row-${index}" class="row row${capitalize(variant)}${row.action ? " rowInteractive" : ""}">${variant === "timeline" ? `<view class="timelineRail"><view class="timelineMarker"></view></view>` : ""}${image}<view class="rowCopy"><text class="rowTitle" value="${escapeMarkup(row.title)}"></text>${row.detail ? `<text class="rowDetail" value="${escapeMarkup(row.detail)}"></text>` : ""}</view>${row.badge ? `<text class="rowBadge" value="${escapeMarkup(row.badge)}"></text>` : ""}${caret}</button>`;
}
...
function actionHierarchy(layout: ActionLayout, index: number, count: number, tone: Tone): string {
  ...
  if (layout === "links") return index === 0 ? "actionWide" : "actionLink";
  return "actionWide";
}
