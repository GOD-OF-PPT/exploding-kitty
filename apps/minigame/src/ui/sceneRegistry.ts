...
const screens: Record<ScreenId, SceneDefinition> = {
  ...
  home: { id: "home", build: (context) => ({ id: "home", eyebrow: COPY.tagline, title: COPY.brand, subtitle: `嗨，${context.view.user.name}`, heroImage: "assets/cats/player.png", heroLabel: "BOOM!", rows: [{ id: "settings", title: "声音与振动", detail: "调整当前设备设置", action: nav("settings", "打开", "settings") }], actions: [nav("play", COPY.start, "play-mode", "yellow"), nav("join", COPY.join, "join", "cream"), nav("tutorial", COPY.tutorial, "tutorial", "cyan"), nav("rules", COPY.rules, "rules", "cream")] }) },
  ...
  create: { id: "create", build: (context) => { const draft = context.roomDraft ?? { maxPlayers: 4, turnSeconds: 45, allowBots: true }; return { id: "create", eyebrow: "PRIVATE ROOM", title: COPY.create, rows: [{ id: "players", title: "玩家人数", detail: `当前 ${draft.maxPlayers} 人 · 点按切换 2 - 5 人`, badge: `${draft.maxPlayers} 人`, action: { id: "cycle-players", label: "切换人数", intent: { type: "CycleRoomPlayers" } } }, { id: "timer", title: "行动计时", detail: "点按切换 30 / 45 / 60 秒", badge: `${draft.turnSeconds} 秒`, action: { id: "cycle-timer", label: "切换计时", intent: { type: "CycleTurnSeconds" } } }, { id: "bots", title: "机器人补位", detail: draft.allowBots ? "允许机器人补位，点按关闭" : "机器人补位已关闭，点按开启", badge: draft.allowBots ? "已开启" : "已关闭", action: { id: "toggle-bots", label: draft.allowBots ? "关闭" : "开启", intent: { type: "ToggleRoomBots" } } }, { id: "ruleset", title: "规则集（固定）", detail: "original-2025@1 · 版本固定", badge: "基础版" }], actions: [intent("create", "创建并邀请", "CreateRoom"), nav("back", "返回", "play-mode", "ink")] }; } },
  join: { id: "join", build: (context) => { const code = context.joinCode?.trim() ?? ""; const valid = /^\d{6}$/u.test(code); return { id: "join", eyebrow: "JOIN THE CHAOS", title: COPY.join, subtitle: "输入好友发来的 6 位房间码", heroImage: "assets/cats/a-ju.png", rows: [{ id: "room-code", title: "房间码", detail: code, badge: valid ? "可加入" : "6 位数字" }], actions: [valid ? intent("join", "进入房间", "JoinRoom") : inactive("join", "进入房间"), nav("back", "返回", "play-mode", "ink")] }; } },
  "lobby-host": { id: "lobby-host", build: (context) => { const playerRows = avatarRows(context, "已准备").map((row) => { const player = context.view.players.find((entry) => entry.id === row.id); return player?.bot ? { ...row, detail: "点击移除这个 Bot", action: intent(`remove-${row.id}`, "移除", "RemoveBot", { playerId: row.id }, "red") } : row; }); const openSeats = Math.max(0, context.view.room.maxPlayers - context.view.players.length); const inviteRows: ScreenRow[] = openSeats ? [{ id: "invite", title: "邀请好友", detail: `还有 ${openSeats} 个空位`, badge: "分享房间码", action: intent("share-seat", "邀请", "ShareRoom", {}, "yellow") }] : []; return { id: "lobby-host", eyebrow: `ROOM #${context.view.room.code}`, title: COPY.lobby, subtitle: "你是房主，所有人准备后即可开始", rows: [...playerRows, ...inviteRows], actions: [intent("share", "分享房间码", "ShareRoom", {}, "yellow"), ...(context.view.room.allowBots && openSeats ? [intent("bot", "加入 Bot", "AddBot", {}, "cream")] : []), ...(context.view.players.length >= 2 && context.view.players.every((player) => player.ready) ? [intent("start", "开始游戏", "StartMatch", {}, "cyan")] : []), intent("leave", "离开房间", "LeaveRoom", {}, "ink")], scroll: true }; } },
  ...
  "other-turn": { id: "other-turn", build: (context) => { const active = context.view.players.find((player) => player.id === context.view.game.turnPlayerId); const seconds = remainingSeconds(context.view.game.deadline, context); return { ...table(context, false), id: "other-turn", subtitle: active ? `等待 ${active.name} 行动 · 还剩 ${seconds} 秒` : `等待其他玩家行动 · 还剩 ${seconds} 秒` }; } },
  attack: { id: "attack", build: (context) => { const owed = Math.max(2, context.view.game.turnsOwed); return { ...table(context, true), id: "attack", eyebrow: `攻击已生效 · 当前第 ${context.view.game.turnNumber} 回合`, title: `你还欠 ${owed} 个回合！`, heroLabel: `×${owed}` }; } },
  response: { id: "response", build: (context) => { const pending = context.view.pending?.kind === "RESPONSE" ? context.view.pending : null; const windowId = pending?.windowId ?? pending?.id ?? ""; const legalNope = context.view.legalActionDetails.find((action) => action.type === "PlayNope" && action.cardTokens?.[0]); const actor = context.view.players.find((player) => player.id === pending?.actorId); const target = context.view.players.find((player) => player.id === pending?.targetId); const types = pending?.cardTypes.length ? pending.cardTypes.map((type) => cardName(type)).join(" + ") : "动作"; const declaration = pending?.declaredCardType ? `，声明 ${cardName(pending.declaredCardType)}` : ""; const targetCopy = target ? `，目标 ${target.name}` : ""; const tableModel = table(context, false).table; return { id: "response", eyebrow: `否决窗口 · ${remainingSeconds(pending?.deadline, context)} 秒`, title: "有人要否决吗？", subtitle: `${actor?.name ?? "一名玩家"}打出了 ${types}${targetCopy}${declaration}。再次否决会让原动作重新生效。`, heroImage: actor?.avatar ?? "assets/cards/card-back.png", heroLabel: `${remainingSeconds(pending?.deadline, context)} 秒`, ...(tableModel ? { table: tableModel } : {}), actions: [...(legalNope && windowId ? [intent("nope", "打出否决", "PlayNope", { cardToken: legalNope.cardTokens![0], windowId }, "red")] : []), ...(can(context, "PassResponse") && windowId ? [intent("pass", "放行 / 关闭", "PassResponse", { windowId }, "cream")] : [])] }; } },
  favor: { id: "favor", build: (context) => { const cards = selectedCards(context); const count = cards.length; const needsDeclaration = count === 3; const eligible = new Set(eligibleTargets(context.view, cards).map((player) => player.id)); const rows: ScreenRow[] = avatarRows(context).filter((row) => eligible.has(row.id)).map((row) => { const selected = row.id === context.selectedTargetId; return { ...row, title: selected ? `✓ ${row.title}` : row.title, detail: selected ? "已选为本次目标" : row.detail, badge: selected ? "已选择" : row.badge, action: { id: `target-${row.id}`, label: selected ? "已选择" : "选择", intent: { type: "SelectTarget", targetId: row.id } } }; }); if (needsDeclaration) rows.push({ id: "declare", title: "声明索要的牌型", detail: "点击循环选择牌型", badge: context.declaredCardType ?? "未选择", action: { id: "declare", label: "切换", intent: { type: "CycleDeclaredCard" } } }); const ready = Boolean(context.selectedTargetId && eligible.has(context.selectedTargetId) && (!needsDeclaration || context.declaredCardType)); const heroImage = count === 1 && cards[0]?.type === "FAVOR" ? "assets/cards/peek.png" : cards[0]?.image ?? "assets/cards/peek.png"; return { id: "favor", eyebrow: count === 1 ? "帮忙 · FAVOR" : count === 2 ? "两张同名组合" : "三张同名组合", title: "选择目标玩家", subtitle: count === 3 ? "选择玩家并声明一种牌" : "对方将交出或失去一张牌", heroImage, rows, actions: [...(ready ? [intent("confirm", "确认目标并出牌", "PlayCards")] : []), nav("back", "取消", "game", "ink")], scroll: true }; } },
  "give-card": { id: "give-card", build: (context) => { const pending = context.view.pending?.kind === "GIVE_CARD" ? context.view.pending : null; const promptId = pending?.promptId ?? pending?.id ?? ""; const requester = context.view.players.find((player) => player.id === pending?.requesterId); const recipient = pending?.requesterName ?? requester?.name ?? "对方玩家"; const card = selectedCards(context)[0]; const allowed = card && context.view.legalActionDetails.some((action) => action.type === "ChooseCard" && action.cardTokens?.includes(card.token)); return { id: "give-card", eyebrow: `秘密选择 · ${remainingSeconds(pending?.deadline, context)} 秒`, title: `交给 ${recipient} 一张牌`, subtitle: card ? `接收者：${recipient} · 已选择：${card.name}` : `接收者：${recipient} · 点击选择要交出的手牌`, heroImage: requester?.avatar, heroLabel: `接收者：${recipient}`, cards: context.view.hand, scroll: true, actions: [...(allowed && promptId ? [intent("give", `交给 ${recipient}`, "ChooseCard", { promptId, cardToken: card.token })] : [])] }; } },
  ...
  defuse: { id: "defuse", build: (context) => { const pending = context.view.pending; const promptId = String(pending && "promptId" in pending ? pending.promptId ?? pending.id : ""); const deckSize = Number(pending?.kind === "DEFUSE_INSERTION" ? pending.deckSize : context.view.game.drawPileCount); const position = Math.max(0, Math.min(deckSize, context.insertionPosition ?? 0)); const positionLabel = position === 0 ? "牌堆顶" : position === deckSize ? "牌堆底" : `第 ${position + 1} 张`; const positionCount = deckSize + 1; return { id: "defuse", eyebrow: `秘密操作 · ${remainingSeconds(pending?.deadline, context)} 秒`, title: "把危险放回哪里？", subtitle: `牌堆共 ${deckSize} 张 · 可选牌堆顶至牌堆底`, heroImage: "assets/cards/defuse.png", rows: [{ id: "position-prev", title: "向牌堆顶移动一格", badge: "向顶", action: { id: "position-prev", label: "向牌堆顶移动一格", intent: { type: "CycleInsertionPosition", delta: -1 } } }, { id: "position-current", title: `当前位置：${positionLabel}`, detail: `第 ${position + 1} / ${positionCount} 个可选位置`, badge: positionLabel }, { id: "position-next", title: "向牌堆底移动一格", badge: "向底", action: { id: "position-next", label: "向牌堆底移动一格", intent: { type: "CycleInsertionPosition", delta: 1 } } }], actions: can(context, "InsertKitten") && promptId ? [intent("insert", `放回 ${positionLabel}`, "InsertKitten", { promptId, position }, "cyan")] : [] }; } },
  eliminated: { id: "eliminated", build: (context) => { const result = context.view.rankings.find((item) => item.playerId === context.view.viewerId); return { id: "eliminated", eyebrow: "砰！", title: "你炸毛了", subtitle: "别灰心，下一局把危险留给他们。", heroImage: "assets/cats/player.png", rows: result ? [{ id: "rank", title: "本局名次", detail: result.reason ?? "已淘汰", badge: `第 ${result.rank} 名` }] : [], actions: [nav("spectate", "继续观战", "other-turn"), intent("leave", "退出房间", "LeaveRoom", {}, "ink")] }; } },
  result: { id: "result", build: (context) => { const byId = new Map(context.view.players.map((player) => [player.id, player])); const ranking = context.view.rankings.length ? [...context.view.rankings] : [{ playerId: context.view.winnerId, rank: 1 }]; const host = context.view.room.ownerId === context.view.viewerId; const voted = context.view.restartVotes.includes(context.view.viewerId); const winner = context.view.players.find((player) => player.id === context.view.winnerId); return { id: "result", eyebrow: "最后一只猫站着", title: "本局结算", subtitle: context.view.restartVotes.length ? `${context.view.restartVotes.length} 位玩家已投票再来一局` : undefined, heroImage: winner?.avatar ?? "assets/cats/tuan-zi.png", heroLabel: context.view.winnerId === context.view.viewerId ? "WINNER · YOU" : "WINNER", rows: ranking.sort((left, right) => left.rank - right.rank).map((item) => { const player = byId.get(item.playerId); const viewer = item.playerId === context.view.viewerId; return { id: item.playerId, title: viewer ? `你 · ${player?.name ?? item.playerId}` : player?.name ?? item.playerId, detail: item.rank === 1 ? viewer ? "你赢得了本局" : "获胜" : item.reason ?? "已淘汰", badge: viewer ? `YOU · #${item.rank}` : `#${item.rank}`, image: player?.avatar }; }), actions: [...(!voted || host ? [intent("restart", host ? "再来一局" : "投票再来一局", host ? "RestartMatch" : "VoteRestart")] : []), intent("leave", "回到首页", "LeaveRoom", {}, "cream")], scroll: true }; } },
  ...
  history: { id: "history", build: (context) => ({ id: "history", eyebrow: "PUBLIC EVENTS", title: "行动记录", rows: context.view.events.map((event, index) => ({ id: `event-${index}`, title: eventTitle(event, index), detail: "公开信息" })), actions: [nav("back", "返回牌桌", "game", "cream")], scroll: true }) },
  "game-menu": { id: "game-menu", build: (context) => ({ id: "game-menu", eyebrow: `ROOM #${context.view.room.code}`, title: "对局菜单", subtitle: can(context, "Concede") ? "认输会结束本局操作，但仍可继续观战。" : undefined, heroImage: "assets/cats/player.png", rows: [{ id: "rules", title: COPY.rules, detail: "卡牌、组合与平台规则", action: nav("rules", "打开", "rules") }, { id: "settings", title: "声音与振动", detail: "只影响当前设备", action: nav("settings", "打开", "settings") }, { id: "network", title: "网络状态", detail: "连接与同步信息", action: nav("network", "打开", "network") }], actions: [nav("back", "返回牌桌", "game", "cream"), ...(can(context, "Concede") ? [intent("concede", "认输并继续观战", "Concede", {}, "red")] : [])] }) },
  network: {
    id: "network",
    build: (context) => {
  ...
  settings: { id: "settings", build: (context) => { const sound = context.settings?.sound !== false; const vibration = context.settings?.vibration !== false; return { id: "settings", eyebrow: "当前设备", title: "设置", heroImage: context.view.user.avatar, heroLabel: context.view.user.name, rows: [{ id: "sound", title: "游戏声音", detail: "卡牌与危险提示音", badge: sound ? "已开启" : "已关闭", action: { id: "toggle-sound", label: sound ? "关闭声音" : "开启声音", intent: { type: "ToggleSound" } } }, { id: "vibration", title: "触感反馈", detail: "出牌与危险提示", badge: vibration ? "已开启" : "已关闭", action: { id: "toggle-vibration", label: vibration ? "关闭触感" : "开启触感", intent: { type: "ToggleVibration" } } }, { id: "tutorial", title: "重看新手教学", detail: "重新查看三步基础教学", action: nav("tutorial", "打开教学", "tutorial") }, { id: "rules", title: "规则与版本", detail: "查看卡牌、组合与平台规则", action: nav("rules", "打开规则", "rules") }, { id: "version", title: "当前版本", detail: "original-2025@1", badge: "基础版" }], actions: [nav("back", "完成", "home", "cream")], scroll: true }; } },
};
...
function table(context: SceneContext, myTurn: boolean): ScreenModel {
  ...
  return {
    ...
    table: { turn: context.view.game.turnNumber, direction: context.view.game.direction, deckCount: context.view.game.drawPileCount, discard, hand: context.view.hand, players: context.view.players, myTurn, turnsOwed: context.view.game.turnsOwed },
    rows: tutorialHint ? [tutorialHint] : undefined,
    actions: myTurn ? [...(can(context, "Draw") && context.view.game.turnId ? [intent("draw", "抽一张", "Draw", { turnId: context.view.game.turnId })] : []), ...(playAction ? [playAction] : []), nav("menu", "菜单", "game-menu", "ink")] : [nav("menu", "菜单", "game-menu", "ink"), nav("history", "行动记录", "history", "cream")],
  };
}
