...
const screens: Record<ScreenId, SceneDefinition> = {
  ...
  "game-menu": { id: "game-menu", build: (context) => ({ id: "game-menu", eyebrow: `ROOM #${context.view.room.code}`, title: "对局菜单", heroImage: "assets/cats/player.png", rows: [{ id: "rules", title: COPY.rules, detail: "卡牌、组合与平台规则", action: nav("rules", "打开", "rules") }, { id: "settings", title: "声音与振动", detail: "只影响当前设备", action: nav("settings", "打开", "settings") }, { id: "network", title: "网络状态", detail: "连接与同步信息", action: nav("network", "打开", "network") }], actions: [...(can(context, "Concede") ? [intent("concede", "认输并继续观战", "Concede", {}, "red")] : []), nav("back", "返回牌桌", "game", "cream")] }) },
